...
$('documnet').ready(function() {
    ...
    $('#borrowModal #confirmBorrow').click(function(e) {
        if (bookId) {
            $.ajax({
                url: 'http://localhost/BMbookmanager/sever/cuihuan.php',
                type: 'post',
    ...
    $('#normalModal #confirmNormal').click(function(e) {
        if (bookId) {
            $.ajax({
                url: 'http://localhost/BMbookmanager/sever/normal.php',
                type: 'post',
                data: { bookid: bookId ,rid: rId},
                success: function(data) {
                    console.log(data);
	...
	function refreshBorrow() {
		$borrowTable.empty();
		$.ajax({
			url: 'http://localhost/BMbookmanager/sever/getborrows.php',
			type: 'POST',
			datatype: 'json',
			// data: {},
			success:function(ss){
				ss.forEach(function( item, index, array){
					...
					var $bwstatus=$('<td>').html(item.status);
					var $borrowtd = $('<td>');
